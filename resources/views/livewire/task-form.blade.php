<div>
    <h1 class="mb-4 text-2xl text-zinc-100">{{ $action }} Task</h1>
    ...
    <form wire:submit='{{ $method }}'>
        ...
        <div class="relative z-0 w-full mb-10 group">
            <textarea name="description" id="description" rows="1" placeholder=" " wire:model='form.description'
        ...
        <div class="relative z-0 w-full mb-5 group">
            <input type="date" name="due_date" id="due_date" wire:model='form.due_date'
                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder=" " />
            ...
            <div class="relative z-0 w-full mb-5 group">
                ...
                <select id="status" wire:model='form.status'
                    ...
                    @foreach ($statuses as $status)
                        <option class="bg-zinc-800" value="{{ $status }}">{{ $status->label() }}</option>
                    @endforeach
                </select>
            </div>
        @endif
